...
`icc = 16`  
p_plant_electric_net_required_mw = 1000  
...
*Depends on blanket concept and thickness, can be a strongly limiting factor for the design space. Proper veryfication requires full 3D neutronics simulations to ensure VV/coil lifetime. According to DEMO 2014: max valiue is 1.35 MW/m2, so it should be corrected according to peaking factor. Proxima assumed 4.05 as peak value for Stellaris, at a cost of only 4 years of full-power operation.*
...
*The exact limit depends on the divertor concept. In stellarators, divertor heat load is not limiting in most cases, unless very agressive assumptions are used for other constrains. 10 MW/M2 is considered avichable for mature reactor divertors.*
...
*For HCPB. This is a limit for total FW heat load, but radiation should make 95% of heat load. The problem with remaining 5% from charged particles is that they will be localized, so local overheating is possible (this would require 3D study in case of stellartors and is outside the PROCESS capabilities)*
 ...
 *Physical limit, Maximal Coil Stress on Ground insulation (approx.) This is the stress limit for the radial force which acts on the coil during operation. (Centering force is not limiting for stellartor build, as there is enough space for the support structure in the machine center)*  
`icc = 32`  
sig_tf_wp_max = 4.0e8  
...
`ixc = 25`               
fp_plant_electric_net_required_mw = 1.0000        
boundl(25) = 0.99  
boundu(25) = 1.0

- Fraction TF coil critical current to operation current  
*In principle it can be made an iteration variable, but we don't reccoment this. This is a safty margin between operational current denisty and superconductors maximum current denisty coming from the material specification (maximum current density as a function of magnetic field and temperature is deined in the PROCESS for diffrent SC types).*  
~~`ixc = 50`  ~~      
fiooic = 0.8  
~~boundu(50) = 0.9~~  
~~boundl(50) = 0.001~~  
...
boundl(56) = 1  
boundu(56) = 100.  
*Usually quench duration is on the order of tens of seconds, but as it should be self-consistantly defined, we apply wide margines to not hinder convergence*

- Coil scaling intoroduced in related paper  
`ixc = 176`  
...
*We don't need current in stellarator.*
...
`tdiv     = 5.0`          
*It is used to calculate ion speed in SOL. It should be around 5-10 eV in detached scenario (Infinity Two assumes 2-10eV). We use 5eV, as this is reasonable when considering plasma closer to divertor plates (in case if this will be used also by some other models)*
...
*The values given here are for HCPB blanket*
...
`f_nd_impurity_electrons(1)  = 1.0`

* Helium (fraction calculated by code)  
`f_nd_impurity_electrons(2)  = 0.1`

* Beryllium  
`f_nd_impurity_electrons(3)  = 0.0`

* Carbon  
`f_nd_impurity_electrons(4)  = 0.0`

* Nitrogen  
`f_nd_impurity_electrons(5)  = 0.0`

* Oxygen  
`f_nd_impurity_electrons(6)  = 0.0`

* Neon  
`f_nd_impurity_electrons(7)  = 0.0`

* Silicon  
`f_nd_impurity_electrons(8)  = 0.0`

* Argon  
`f_nd_impurity_electrons(9)  = 0.0`

* Iron  
`f_nd_impurity_electrons(10) = 0.0`

* Nickel  
`f_nd_impurity_electrons(11) = 0.0`

* Krypton  
`f_nd_impurity_electrons(12) = 0.0`

* Xenon  
`f_nd_impurity_electrons(13) = 0.0`

* Tungsten  
`f_nd_impurity_electrons(14) = 1.0E-5`
...
*Number of iterations should depend on case specification. For well defined constrains, 10-30 iterations should be enough, but some cases require much more iterations and increasing the limit to 100 is not uncommon.*  
`maxcal   = 50`
...
`tftmp     = 4.5`  
*Critial temperature of TF coils is ≈6K. The safty margin is estimated as 0.5K, but we don't know the exact termperature distribution inside the coil. We assume that the peak temperature is up to 1K higher than inlet temperature (4,5K at inlet). The total margin is 1.5K.*

* Temperature margin for superconductor current denisty calculation (K)
tmargmin = 1.5
...
* Quench detection+activation time (or dump delay) (s)
